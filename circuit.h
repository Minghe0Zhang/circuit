...
enum GATE_TYPE {INPUT=0, OUTPUT=1, LATCH_IN=11, LATCH_OUT=12, T_BUFF=2, T_NOT=3, T_AND=4, T_NAND=5, T_OR=6, T_NOR=7, T_XOR=8, T_0=9, T_1=10, T_UN= 13};
...
typedef struct GATE{
    ...
    int gateID;
    unsigned faninSize;
    unsigned pseudo_node_ID;
    unsigned gate_node_ID;
    unsigned type, level;
    float cap;
    unsigned fanoutNum, faninNum;
    float delay,delay1;
    vector<unsigned> fanout_arr;
    vector<unsigned> fanin_arr;
    float D1, D2;
    unsigned retiming_type;
    // for output latches
    vector<unsigned> g_t_ei_id;
    unsigned t_ei_id;
    float edge_mult_by;
...
typedef struct NODE{
    ...
    int out_degree;
    int type;
    vector<EDGE> fanout_node;
    vector<EDGE> fanin_node;
} node;
...
public:
    ...
    float edl_cost;
    float index[7], rising[6][7], falling[6][7];
	//graph
    ...
    void  create_psuedo_node();
	void  create_gate_node();
    float libDelay(unsigned ID, bool ris);	//void  create_psuedo_t();
	/* node graph*/
	void  output_node_graph(char* constraint_name,float t);
    //data processing
    void  get_r_u(char* flow_info_name,char* answer_name,int cal_ori);
};
